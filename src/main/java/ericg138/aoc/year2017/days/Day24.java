...
import ericg138.aoc.year2017.beans.Bridge;
import ericg138.aoc.year2017.beans.Component;
...
public class Day24 extends Day {
    ...
    private final int maxStength;
    private final int strengthLongest;

    public Day24() {
        super(24);

        Set<Component> components = getInputAsList().stream()
                .map(line -> new Component(Integer.parseInt(line.substring(0, line.indexOf('/'))),
                        Integer.parseInt(line.substring(line.indexOf('/') + 1)))).collect(Collectors.toSet());
